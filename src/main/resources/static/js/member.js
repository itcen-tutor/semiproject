...
const submitLoginfrm = async (frm, token, headerName) => {
    ...
    fetch('/api/v1/member/login', { // jwt에서는 다시 재설정
    //fetch('/member/login', { 
        method: 'POST',
        headers: { [headerName]: token },
        body: formData
    }).then(async response => {
        if (response.ok) { // 로그인이 성공했다면 302 라다이렉트 응답 전송
            let data = await response.json();
            alert(data.message); // "로그인 성공했습니다!!"
            sessionStorage.setItem('jwt', data.token); // JWT 저장
